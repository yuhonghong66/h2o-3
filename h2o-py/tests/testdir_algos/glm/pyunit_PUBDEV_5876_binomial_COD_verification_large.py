...
def test_glm_binomial():
    ...
    y = trainF.ncol-1
    x = list(range(y))
    m_LS = glm(family='binomial', seed=12345, solver="coordinate_descent")
    m_LS.train(training_frame=trainF, x=x, y=y)

    mojoZip = pyunit_utils.locate("bigdata/laptop/glm/GLM_model_python_1543520565753_1.zip")
    ...
    pyunit_utils.compare_frames_local(predict_h2o, pred_mojo)
